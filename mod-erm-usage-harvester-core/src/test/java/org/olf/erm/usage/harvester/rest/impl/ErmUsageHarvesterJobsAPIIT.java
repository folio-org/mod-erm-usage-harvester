...
public class ErmUsageHarvesterJobsAPIIT {
  ...
  public void testReturnedData() {
    JobInfos getAllJobInfos = new ApiRequest().withParams(PARAM_LIMIT, 1000).send();
    assertThat(getAllJobInfos).usingRecursiveComparison().isEqualTo(expectedJobInfos);
  }

  @Test
  public void testPaging() {
    JobInfos getPage1 = new ApiRequest().send();
    assertThat(getPage1).satisfies(hasResultSizes(10, expectedJobInfos.getTotalRecords()));

    JobInfos getPage2 = new ApiRequest().withParams(PARAM_OFFSET, 10).send();
  ...
  public void testQueryParam() {
    JobInfos result = new ApiRequest().withParams(PARAM_QUERY, "type==tenant").send();
    assertThat(result)
  ...
  public void testTimestampParam() {
    JobInfos result = new ApiRequest().withParams(PARAM_TIMESTAMP, "1663150477573").send();
    assertThat(result)
        .satisfies(
            hasResultSizes(2, 2),
            containsIds(
